<x-guests-layout>
    ...
    <section class="event min-h-screen w-full p-2 flex items-center justify-center flex-col">
        ...
        <h1 class="font-bold text-center text-xl my-2">{{ $event->name }}</h1>
        <div hidden id="event_id">{{$event->id}}</div>
        ...
        <fieldset id="group_form" class="flex flex-col items-start w-full sm:w-2/3 p-4 border-2 border-secondary-color rounded-md mt-4">
            <legend class="px-2 font-bold">Group Registration:</legend>
            <form id="group-form" class="w-full" method="POST" action="{{ route('events.store.group') }}" enctype="multipart/form-data">
                @csrf
    ...
    <script>
        ...
        $('#type').on('change', function() {
           ...
           const event_id = document.querySelector('div[id=event_id]').textContent;
           if(type === 'individual') {
                window.location = '/events/register/individual/'+event_id;
           }else{
